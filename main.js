...
(function () {
  ...
  if (hasScrollTrigger) {
    try { gsap.registerPlugin(ScrollTrigger); } catch (e) { hasScrollTrigger = false; }
  }

  /* ------------------------------------------------------------------ */
  /* 1. SPLITTING — wraps [data-splitting] into .line > .line__inner     */
  /*    Implemented directly (no external Splitting.js dependency) so    */
  /*    there is one less network request that could ever fail/conflict.*/
  /*    Matches the site's own JP() helper: group already-wrapped words  */
  /*    by their offsetTop into <span class="line"><span class="line__inner">*/
  /* ------------------------------------------------------------------ */
  function initSplitting() {
    document.querySelectorAll("[data-splitting]").forEach(function (el) {
      try { rebuildLines(el); } catch (e) { /* leave element's text as-is */ }
    });
  }
  ...
  /* ------------------------------------------------------------------ */
  /* BOOT — every stage is isolated in its own try/catch so a single     */
  /* failure (blocked CDN, unsupported API, etc.) can never take the     */
  /* rest of the page down or leave it invisible.                        */
  /* ------------------------------------------------------------------ */
  function safe(fn, label) {
    try { fn(); } catch (e) { console.warn("[nova-studio]", label, "failed:", e); }
  }

  function boot() {
    document.body.classList.remove("is-loading");

    safe(initSplitting, "initSplitting");

    requestAnimationFrame(function () {
      safe(initScrollEffects, "initScrollEffects");
      safe(initMarquees, "initMarquees");
      safe(initFooterTrail, "initFooterTrail");
      safe(initBenefitsStack, "initBenefitsStack");
    });
    ...
    window.addEventListener("load", function () {
      safe(function () { if (hasScrollTrigger) ScrollTrigger.refresh(); }, "ScrollTrigger.refresh");
    });
  }

  // Hard safety net: no matter what happens above, the page must never
  // stay hidden. This runs immediately, before any library has a chance
  // to fail, and again as a delayed backstop.
